...
const getUtils = (node, jsdoc, jsdocNode, settings, report, context, iteratingAll, ruleConfig, indent) => {
  ...
  utils.getClassJsdoc = () => {
    ...
    if (classJsdocNode) {
      ...
      return (0, _jsdoccomment.parseComment)(classJsdocNode, indnt);
    }
...
const iterateAllJsdocs = (iterator, ruleConfig) => {
  ...
  const callIterator = (context, node, jsdocNodes, state, lastCall) => {
    ...
    jsdocNodes.forEach(jsdocNode => {
      ...
      iterate(null, indent, jsdoc, ruleConfig, context, lines, jsdocNode, node, settings, sourceCode, iterator, state, true);
    });
    ...
    create(context) {
      ...
      return {
        '*:not(Program)'(node) {
          ...
          const comment = (0, _jsdoccomment.getJSDocComment)(sourceCode, node, settings);
  ...
  return {
    ...
    create(context) {
      ...
      if (ruleConfig.contextDefaults || ruleConfig.contextSelected) {
        ...
        if ((_contexts = contexts) !== null && _contexts !== void 0 && _contexts.includes('any')) {
      ...
      if (contexts && (ruleConfig.contextDefaults || ruleConfig.contextSelected)) {
        ...
        ['ArrowFunctionExpression', 'FunctionDeclaration', 'FunctionExpression'].forEach(prop => {
